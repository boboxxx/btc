...
US_TREND_ENTRY_THRESHOLD = 0.60
US_TREND_CONFIRM_THRESHOLD = 0.54
US_TREND_CONFIRM_REQUIRED = 1
US_TREND_HOLD_PROBABILITY = 0.46
US_TREND_HOLD_SMOOTH = 0.50
US_TREND_HOLD_SOFT_EXIT = 0.18
US_TREND_HOLD_MIN_RANGE_POS = 0.40
US_STOP_LOSS = 0.006
US_PROFIT_THRESHOLD = 0.0006
...
CN_TREND_HOLD_PROBABILITY = 0.22
CN_TREND_HOLD_SMOOTH = 0.34
CN_TREND_HOLD_SOFT_EXIT = 0.05
CN_TREND_HOLD_MIN_RANGE_POS = 0.42
CN_STOP_LOSS = 0.006
CN_PROFIT_THRESHOLD = 0.0008
CN_MIN_EXPECTED_EDGE = 0.0012
CN_OPEN_MIN_EXPECTED_EDGE = 0.0016
EDGE_LOOKBACK_ROWS = 180
EDGE_BANDWIDTH = 0.08
EDGE_MIN_NEIGHBORS = 18
MODEL_REFIT_STRIDE = 5
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output", default=str(root_dir / "data/signals/combined/five_minute_signal_results.xlsx"))
    parser.add_argument("--live-output", default=str(root_dir / "data/signals/combined/five_minute_live_signals.csv"))
    parser.add_argument("--state-output", default=str(root_dir / "data/signals/combined/five_minute_signal_state.csv"))
    parser.add_argument("--history-output", default=str(root_dir / "data/signals/combined/five_minute_signal_history.csv"))
...
def is_us_bullish_regime(row: pd.Series | None) -> bool:
    if row is None:
        return False
    qqq_ret_5 = float(row.get("qqq_ret_5", np.nan))
    nq_ret_5 = float(row.get("nq_ret_5", np.nan))
    qqq_sma_ratio_8 = float(row.get("qqq_sma_ratio_8", np.nan))
    nq_sma_ratio_8 = float(row.get("nq_sma_ratio_8", np.nan))
    qqq_ew = float(row.get("qqq_ew_thrust_5", np.nan))
    nq_ew = float(row.get("nq_ew_thrust_5", np.nan))
    qqq_fib_pos_13 = float(row.get("qqq_fib_pos_13", np.nan))
    checks = [qqq_ret_5, nq_ret_5, qqq_sma_ratio_8, nq_sma_ratio_8, qqq_ew, nq_ew, qqq_fib_pos_13]
    if any(pd.isna(value) for value in checks):
        return False
    return (
        qqq_ret_5 > 0.004
        and nq_ret_5 > 0.004
        and qqq_sma_ratio_8 > 1.001
        and nq_sma_ratio_8 > 1.001
        and qqq_ew > -0.10
        and nq_ew > -0.10
        and qqq_fib_pos_13 < 0.92
    )


def is_cn_bullish_regime(row: pd.Series | None) -> bool:
    if row is None:
        return False
    cnetf_sma_ratio_5 = float(row.get("cnetf_sma_ratio_5", np.nan))
    nq_sma_ratio_5 = float(row.get("nq_sma_ratio_5", np.nan))
    cnetf_ew = float(row.get("cnetf_ew_thrust_5", np.nan))
    nq_ew = float(row.get("nq_ew_thrust_5", np.nan))
    open_to_now_ret = float(row.get("open_to_now_ret", np.nan))
    range_pos = float(row.get("range_pos_so_far", np.nan))
    checks = [cnetf_sma_ratio_5, nq_sma_ratio_5, cnetf_ew, nq_ew, open_to_now_ret, range_pos]
    if any(pd.isna(value) for value in checks):
        return False
    return (
        cnetf_sma_ratio_5 > 1.0005
        and nq_sma_ratio_5 > 1.0005
        and cnetf_ew > -0.20
        and nq_ew > -0.20
        and open_to_now_ret > 0.0015
        and range_pos >= CN_TREND_HOLD_MIN_RANGE_POS
    )


def is_us_bearish_overlay(overlay_reason: str) -> bool:
    return overlay_reason in {"us_bear_breakdown", "us_bear_wave4_low", "us_intraday_exhaustion"}


def is_cn_bearish_overlay(overlay_reason: str) -> bool:
    return overlay_reason in {"cn_downtrend_follow", "cn_intraday_distribution", "cn_corrective_rebound_exit"}


def should_hold_us_trend(
    row: pd.Series | None,
    probability: float,
    smooth_probability: float,
    overlay_reason: str,
) -> bool:
    if is_us_bearish_overlay(overlay_reason):
        return False
    if not is_us_bullish_regime(row):
        return False
    open_to_now_ret = float(row.get("open_to_now_ret", np.nan)) if row is not None else np.nan
    range_pos = float(row.get("range_pos_so_far", np.nan)) if row is not None else np.nan
    if pd.isna(open_to_now_ret) or pd.isna(range_pos):
        return False
    return (
        range_pos >= US_TREND_HOLD_MIN_RANGE_POS
        and open_to_now_ret > 0.0025
        and (probability >= US_TREND_HOLD_PROBABILITY or smooth_probability >= US_TREND_HOLD_SMOOTH)
    )


def should_hold_cn_trend(
    row: pd.Series | None,
    probability: float,
    smooth_probability: float,
    overlay_reason: str,
) -> bool:
    if is_cn_bearish_overlay(overlay_reason):
        return False
    if smooth_probability >= 0.60:
        return True
    if not is_cn_bullish_regime(row):
        return False
    return probability >= CN_TREND_HOLD_PROBABILITY or smooth_probability >= CN_TREND_HOLD_SMOOTH


def position_age_after_transition(current_state: int, next_state: int, current_age: int) -> int:
    if next_state == 0:
        return 0
    if current_state == 1:
...
def live_position_age_bars(
    valid_live: pd.DataFrame,
    current_state: int,
    prior_position_since: pd.Timestamp | None,
) -> int:
    if current_state != 1 or pd.isna(prior_position_since):
        return 0
    anchor = pd.Timestamp(prior_position_since)
    return int(valid_live.loc[valid_live["datetime"] >= anchor].shape[0])
...
def entry_signal_allowed(
    ...
    *,
    ...
    if market == "美股":
        ...
        if not entry_allowed and is_us_bullish_regime(row):
            entry_allowed = (
                probability >= US_TREND_ENTRY_THRESHOLD
                and smooth_probability >= US_TREND_CONFIRM_THRESHOLD
                and confirm_count >= US_TREND_CONFIRM_REQUIRED
            )
...
def walk_forward_5m(dataset: pd.DataFrame, market: str, entry_threshold: float, exit_threshold: float, stop_loss: float) -> FiveMinArtifacts:
    ...
    for i in range(min_train, len(dataset)):
        should_refit = (not model_ready) or ((i - min_train) % MODEL_REFIT_STRIDE == 0)
        if should_refit:
        ...
        if cooldown > 0:
            ...
            if market.startswith("美股") and should_hold_us_trend(row.iloc[0], probability, smooth_probability, overlay_reason) and probability > US_TREND_HOLD_SOFT_EXIT:
                next_position = 1
            elif market.startswith("场内") and should_hold_cn_trend(row.iloc[0], probability, smooth_probability, overlay_reason) and probability > CN_TREND_HOLD_SOFT_EXIT:
                next_position = 1
            else:
                next_position = 0
        ...
        if next_position == 1:
            ...
            if pd.notna(low_dd) and low_dd < -stop_loss:
                ...
                next_position = 0
                cooldown = 1
        next_position_age = position_age_after_transition(current_position, next_position, current_position_age)
        ...
        current_position_age = next_position_age
...
def live_row_from_frame(
    ...
    if market_phase != "midday_break":
        if current_state == 0 and entry_allowed:
            ...
            if market == "美股" and should_hold_us_trend(latest.iloc[0], probability, smooth_probability, overlay_reason) and probability > US_TREND_HOLD_SOFT_EXIT:
                recommended_state = 1
            elif market == "场内" and should_hold_cn_trend(latest.iloc[0], probability, smooth_probability, overlay_reason) and probability > CN_TREND_HOLD_SOFT_EXIT:
                recommended_state = 1
            else:
                recommended_state = 0
    ...
    current_position_age = live_position_age_bars(valid_live, current_state, prior_position_since)
    ...
    if action == "买入":
        ...
        if pd.notna(prior_position_since):
            ...
            position_since = feature_bar_time
            position_since_exact = 1
        position_age_bars = max(current_position_age, 1)
    elif action == "卖出":
        last_action_time = feature_bar_time
    ...
    return {
        ...
        "position_age_bars": int(position_age_bars),
...
def main(argv: list[str] | None = None) -> None:
    ...
    if need_us:
        ...
        us_dataset = make_intraday_dataset(
            qqq_raw,
            nq_features_us,
            "qqq",
            "美股",
            label_horizon=3,
            stop_loss=US_STOP_LOSS,
            profit_threshold=US_PROFIT_THRESHOLD,
        )
    ...
    if need_cn:
        ...
        cn_dataset = make_intraday_dataset(
            cn_raw,
            nq_features_cn,
            "cnetf",
            "场内",
            label_horizon=3,
            stop_loss=CN_STOP_LOSS,
            profit_threshold=CN_PROFIT_THRESHOLD,
        )
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        notes = pd.DataFrame(
            [
                ...
                ("us_stop_loss", f"{US_STOP_LOSS:.3f}"),
                ("us_profit_threshold", f"{US_PROFIT_THRESHOLD:.4f}"),
                ("us_trend_hold_probability", f"{US_TREND_HOLD_PROBABILITY:.2f}"),
                ("us_trend_hold_smooth", f"{US_TREND_HOLD_SMOOTH:.2f}"),
                ("us_trend_hold_soft_exit", f"{US_TREND_HOLD_SOFT_EXIT:.2f}"),
                ("cn_stop_loss", f"{CN_STOP_LOSS:.3f}"),
                ("cn_profit_threshold", f"{CN_PROFIT_THRESHOLD:.4f}"),
                ("cn_trend_hold_probability", f"{CN_TREND_HOLD_PROBABILITY:.2f}"),
                ("cn_trend_hold_smooth", f"{CN_TREND_HOLD_SMOOTH:.2f}"),
                ("cn_trend_hold_soft_exit", f"{CN_TREND_HOLD_SOFT_EXIT:.2f}"),
                ("model_refit_stride", str(MODEL_REFIT_STRIDE)),
                ("us_trend_override_rule", "US entries can relax to a lower threshold when QQQ/NQ are already in a confirmed bullish intraday trend regime."),
                ("us_trend_hold_rule", "US exits are delayed when QQQ/NQ remain in a confirmed bullish intraday regime and probability only softens without a bearish overlay breakdown."),
                ("cn_trend_hold_rule", "CN exits are delayed when the ETF and NQ remain in a confirmed bullish intraday regime and the weakening probability has not reached a true bearish breakdown."),
