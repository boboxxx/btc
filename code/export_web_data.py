...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--us-live", default=str(ROOT_DIR / "data/signals/us/us_qqq_live_signals.csv"))
    parser.add_argument("--us-history", default=str(ROOT_DIR / "data/signals/us/us_qqq_signal_history.csv"))
    parser.add_argument("--cn-live", default=str(ROOT_DIR / "data/signals/cn/cn_nasdaq_live_signals.csv"))
    parser.add_argument("--cn-history", default=str(ROOT_DIR / "data/signals/cn/cn_nasdaq_signal_history.csv"))
    parser.add_argument("--five-minute-live", default=str(ROOT_DIR / "data/signals/combined/five_minute_live_signals.csv"))
    parser.add_argument("--five-minute-history", default=str(ROOT_DIR / "data/signals/combined/five_minute_signal_history.csv"))
    parser.add_argument("--btc-live", default=str(ROOT_DIR / "data/signals/btc/btc_live_signals.csv"))
    parser.add_argument("--btc-history", default=str(ROOT_DIR / "data/signals/btc/btc_signal_history.csv"))
    parser.add_argument("--open-nowcast-live", default=str(ROOT_DIR / "data/open_nowcast/open_nowcast_live.csv"))
    parser.add_argument("--output", default=str(ROOT_DIR / "docs/data/dashboard.json"))
...
def build_payload(args: argparse.Namespace) -> dict[str, object]:
    us_live = read_csv_if_exists(Path(args.us_live))
    us_history = read_csv_if_exists(Path(args.us_history))
    cn_live = read_csv_if_exists(Path(args.cn_live))
    cn_history = read_csv_if_exists(Path(args.cn_history))
    ...
    use_split_intraday = not us_live.empty or not cn_live.empty
    use_split_history = not us_history.empty or not cn_history.empty
    intraday_parts = [us_live, cn_live] if use_split_intraday else [five_minute_live]
    history_parts = [us_history, cn_history] if use_split_history else [five_minute_history]
    combined_live = pd.concat([*intraday_parts, btc_live], ignore_index=True, sort=False)
    combined_history = pd.concat([*history_parts, btc_history], ignore_index=True, sort=False)
    ...
    return {
        ...
        "sources": {
            "us_live_exists": not us_live.empty,
            "us_history_exists": not us_history.empty,
            "cn_live_exists": not cn_live.empty,
            "cn_history_exists": not cn_history.empty,
